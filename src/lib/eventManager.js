...
export function setupEventListeners(root) {
  ...
  Array.from(eventMap.keys()).forEach((event) => {
    // 2. 기존에 설정된 이벤트 리스너 제거 (있다면)
    removeEvent(rootElement, event, handleEvent)

    // 3. eventMap에 등록된 모든 이벤트 타입에 대해 루트 요소에 이벤트 리스너 추가
    // 주의: 이벤트 캡처링을 사용하여 이벤트를 상위에서 하위로 전파
    rootElement.addEventListener(event, handleEvent, { capture: true })
  })
}
...
export function addEvent(element, eventType, handler) {
  ...
  rootElement && rootElement.addEventListener(eventType, handleEvent, true)
}
