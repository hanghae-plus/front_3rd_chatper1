...
function handleEvent(event) {
  ...
  const typeMap = eventMap.get(event.type);

  while (target && target !== rootElement) {
    if (typeMap.has(target)) {
      const handlers = typeMap.get(target);
      if (handlers) {
...
export function addEvent(element, eventType, handler) {
  ...
  const typeMap = eventMap.get(eventType);
  if (!typeMap.has(element)) {
    typeMap.set(element, []);
  }

  typeMap.get(element).push(handler);
}
...
export function removeEvent(element, eventType, handler) {
  ...
  const typeMap = eventMap.get(eventType);
  if (typeMap && typeMap.has(element)) {
