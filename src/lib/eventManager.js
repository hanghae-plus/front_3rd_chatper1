...
export const setupEventListeners = (root) => {
  ...
  [...eventMap.keys()].forEach((eventType) => {
    ...
    rootElement.removeEventListener(eventType, handleEvent); // 2. 기존에 설정된 이벤트 리스너 제거 (있다면)
    rootElement.addEventListener(eventType, handleEvent); // 3. eventMap에 등록된 모든 이벤트 타입에 대해 루트 요소에 이벤트 리스너 추가
  });
};
...
export const addEvent = (eventType, element, handler) => {
