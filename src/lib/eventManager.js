...
export function setupEventListeners(root) {
  ...
  // rootElement 설정
  rootElement = root;
  // eventMap에 등록된 모든 이벤트 타입에 대해 루트 요소에 이벤트 리스너 추가
  eventMap.forEach((_, eventType) => {
...
function handleEvent(event) {
  ...
  // 이벤트 타겟에서 시작하여 루트 요소까지 버블링
  while ($element && $element !== rootElement) {
    // 각 요소에 대해 해당 이벤트 타입의 핸들러가 있는지 확인
...
export function addEvent(element, eventType, handler) {
  ...
  // * 멘토링 이후 2번 적용 안 하는 것으로 결정. (루트 접근은 setupEventListeners에서만 관리)

  // eventMap에 이벤트 타입과 요소, 핸들러 정보 저장
