...
function updateElement(parent, newVNode, oldVNode, index = 0) {
  console.log(newVNode, oldVNode, parent, 'newVNode, oldVNode, container');
  ...
  if (typeof newVNode === 'string' || typeof oldVNode === 'string') {
    if (newVNode !== oldVNode) {
      parent.replaceChild(document.createTextNode(newVNode), parent.childNodes[index]);
    }
    return;
  }
  ...
  if (newVNode.type === oldVNode.type) {
    updateAttributes(newVNode, oldVNode);
  }
  ...
  for (let i = 0; i < maxLength; i++) {
    updateElement(parent.childNodes[index], newVNode.children[i], oldVNode.children[i], i);
  }
  // 5-3. 불필요한 자식 노드 제거
...
export function renderElement(vNode, container) {
  ...
  if (!container.firstChild) {
    ...
    return (container.__vNode = processVNode(vNode));
  }

  console.log(container.firstChild, container.__vNode, 'container.firstChild');

  updateElement(container, processVNode(vNode), container.__vNode);
